...
# In the actual version of scales package there is an issue that doesn't allow to use the function dollar() with the scale_cut parameter.
# For more info see https://github.com/r-lib/scales/issues/413
# In the meantime, I've created a workaround that allows to use the scale_cut parameter with the dollar() function.
...
number <- function(
  ...
  style_positive <- rlang::arg_match(style_positive, c("none", "plus", "space"))
  style_negative <- rlang::arg_match(
    style_negative,
    c("hyphen", "minus", "parens")
  )
...
scale_cut <- function(x, breaks, scale = 1, accuracy = NULL, suffix = "") {
  # rlang::check_object(breaks, is.numeric, "a numeric vector", arg = caller_arg(breaks))
  ...
  bad_break <- rlang::`%|%`(
    ((x * scale / breaks[break_suffix]) %% 1 != 0),
    FALSE
  )
  if (any(bad_break)) {
